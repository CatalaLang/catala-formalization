...
Inductive inv_scope_root: type -> Prop :=
  | InvScopeLet:
    forall T1 T2,
      inv_root T1 ->
      inv_root T2 ->
      inv_scope_root (TFun T1 T2)
  | InvRoot:
    forall T,
      inv_root T ->
      inv_scope_root T
...
Ltac2 sinv_inv () :=
  match! goal with
  | [ h : inv_scope_root ?c |- _ ] => smart_inversion c h
...
Module Typing_Examples.
  ...
  *)
  ...
  Proof.
    ...
    all: eapply InvRoot; repeat econs_inv.
  Qed.
...
Proof.
  induction 1.
  all: try solve [econs_inv; eauto|eauto].
Qed.
...
Proof.
  ...
  8:{ repeat (econstructor; eauto). }
  8:{ (* Returning an Conflict *)
