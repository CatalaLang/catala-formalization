...
Inductive sym_expr :=
  ...
  | Sclo (t: term) (sigma : list sym_expr)
  .
...
Fixpoint eval_sym_expr (env : string -> value) (e : sym_expr) : value :=
  match e with
  | Sbool b => Bool b
  | Sint i => Int i
  | Svar s => env s
  | Sop Add e1 e2 =>
      match eval_sym_expr env e1, eval_sym_expr env e2 with
  ...
  | Sclo t l => Closure t (map (eval_sym_expr env) l)
  end.
...
Inductive sym_cont :=
  ...
  | SCClosure (t_cl: {bind term}) (sigma_cl: list sym_expr)
...
(** Symbolic stores *)
Definition sym_store := list sym_expr.

Fixpoint eval_sym_constraint (env : string -> value) (c : sym_constraint) :=
  match c with
  | Eq e1 e2 => value_eqb (eval_sym_expr env e1) (eval_sym_expr env e1)
  | IsTrue e =>
      match eval_sym_expr env e with
      | Bool b => b
      | _ => false
      end
  | And c1 c2 =>
...
Inductive sym_cred: sym_state -> sym_state -> Prop :=
  ...
  | sym_cred_defaultbasetrue:
      ...
      (* make the assymption that symbolic value v is of boolean structure *)
      sym_cred
...
(**
    ...
    to be related with respect to an assignement of free variables
*)
Definition similar_env (env0 : string -> value) (env1 : list value) (env2 : list sym_expr) :=
  Forall2 (similar_value env0) env1 env2.
...
Theorem sym_cred_sound:
  forall sym_s1 sym_s2,
    ...
    forall env0 s1,
      similar env0 s1 sym_s1 ->
      exists s2, similar env0 s2 sym_s2 /\ cred s1 s2.
Proof.
  intros * Hsym * Hsim.
  inversion Hsym; subst.
  - inversion Hsim; subst.
    repeat econstructor; eauto.
    admit arthur "lemma about forall2".
  - inversion Hsim; subst.
    repeat econstructor; eauto.
  - inversion Hsim; subst.
    ...
    f_equal. generalize H6. clear.
    induction 1; auto.
    rewrite IHForall2; simpl.
    now rewrite H.
  - admit arthur "later".
  - admit arthur "later".
  - admit arthur "later".
...
Definition concretize_env (env : string -> value) (ctx : list sym_expr) :=
